...
def SLOPE(S, N):  # 返S序列N周期回线性回归斜率
    return (
        pd.Series(S)
        .rolling(N)
        .apply(lambda x: np.polyfit(range(N), x, deg=1)[0], raw=True)
        .values
    )
...
def DMI(CLOSE, HIGH, LOW, M1=14, M2=6):  # 动向指标：结果和同花顺，通达信完全一致
    TR = SUM(
        MAX(MAX(HIGH - LOW, ABS(HIGH - REF(CLOSE, 1))), ABS(LOW - REF(CLOSE, 1))), M1
    )
    HD = HIGH - REF(HIGH, 1)
    LD = REF(LOW, 1) - LOW
...
def BRAR(OPEN, CLOSE, HIGH, LOW, M1=26):  # BRAR-ARBR 情绪指标
    ...
    BR = (
        SUM(MAX(0, HIGH - REF(CLOSE, 1)), M1)
        / SUM(MAX(0, REF(CLOSE, 1) - LOW), M1)
        * 100
    )
    return AR, BR
